...
const Switch = React.forwardRef(({ className, style, ...props }, ref) => (
  <SwitchPrimitives.Root
    ...
    <SwitchPrimitives.Thumb
      className={cn(
        // Changed bg-white to a deep color (#1F3A4B is your dark background)
        // This ensures it is always darker than the neon green toggle track
        "bg-[#1F3A4B] pointer-events-none block h-5 w-5 rounded-full shadow-lg ring-0 transition-transform data-[state=checked]:translate-x-5 data-[state=unchecked]:translate-x-0",
      )}
    />
  </SwitchPrimitives.Root>
));
